...
const MasterBets = () => {
  ...
  const getTransactions = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${server}api/v1/bet/bets?userId=${userId}`,
        {
          headers: {
  ...
  return (
    <div className="container mx-auto pb-8">
      <h1 className="text-xl font-semibold mb-2">Showing bets of <span className="underline uppercase">{userName}</span> </h1>
          ...
          <tbody className="divide-y divide-[rgb(var(--color-border))] bg-[rgb(var(--color-background))]">
            {currentItems.map((bet) => {
              ...
              return (
                <tr
                  key={bet._id}
                  className="hover:bg-[rgb(var(--color-background-hover))] transition-colors"
                >
                  <td className="px-6 py-4 text-sm text-[rgb(var(--color-text-primary))]">
                    {bet.match}
                  </td>
          ...
          {[...Array(totalPages)].map((_, index) => {
            ...
            if (
              ...
              return (
                <span
                  key={pageNumber}
                  className="px-3 py-1 text-[rgb(var(--color-text-primary))]"
                >
                  ...
                </span>
              );
